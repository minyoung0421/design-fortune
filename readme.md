...
기존 Figma-to-Code 도구들과의 구체적 비교입니다.

| 기능 | **Design Fortune** | Figma Tokens (Jan Six) | Design Tokens Studio | Style Dictionary (Amazon) |
|------|--------------------|------------------------|----------------------|--------------------------|
| Figma 실시간 연동 | ✅ MCP WebSocket | ❌ 수동 플러그인 내보내기 | ❌ 수동 내보내기 | ❌ 수동 JSON 관리 |
| AI 에이전트 통합 | ✅ `DRAGME.md` 컨텍스트 주입 | ❌ 없음 | ❌ 없음 | ❌ 없음 |
| 표준 프로토콜 | ✅ MCP (Anthropic 표준) | ❌ 독자 플러그인 API | ❌ 독자 포맷 | ✅ W3C Draft 호환 |
| CI 자동 검증 | ✅ GitHub Actions 내장 | ❌ 없음 | ❌ 없음 | ⚠️ 별도 설정 필요 |
| 온보딩 복잡도 | **낮음** — `DRAGME.md` 1파일 | 중간 — 플러그인 설치 필요 | 높음 — 플러그인 + Studio | 높음 — CLI + config |
| 하드코딩 차단 | ✅ PR 머지 전 자동 100% 차단 | ❌ 없음 | ❌ 없음 | ⚠️ 별도 linting 필요 |

### 핵심 차별화 포인트 3가지

1. **MCP 표준 프로토콜**: Anthropic이 제정한 Model Context Protocol 기반으로, 특정 AI 플랫폼에 종속되지 않고 모든 MCP 호환 에이전트와 연동 가능
2. **Zero-config AI**: `DRAGME.md` 단일 파일로 AI 에이전트가 즉시 토큰 원칙을 습득 — 프롬프트 엔지니어링 불필요
3. **설계-검증 일체화**: 토큰 정의(`tokens.ts`) → 컴포넌트 적용 → CI 하드코딩 검증이 단일 저장소 내에서 완결
